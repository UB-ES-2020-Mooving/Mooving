...
def test_profile_modify_correct():
...
def test_profile_modify_error1(): # Cuando no se encuentra el cliente de la url
    with app.test_client() as c:
        # Hacemos el put con datos inventados
...
def test_profile_modify_error2(): # El cliente introduce un nuevo email ya en uso
    with app.test_client() as c:
        # Cogemos un cliente random de la base de datos.
        client_old = ClientModel.query.filter(ClientModel.client_id == 1).first()
...
def test_profile_modify_error3():  # El cliente introduce un nuevo dni_nie ya en uso
    with app.test_client() as c:
        # Cogemos un cliente random de la base de datos.
        client_old = ClientModel.query.filter(ClientModel.client_id == 1).first()
